...
const Rooms = () => {
  ...
  const handleBookRoom = (roomNumber) => {
    navigate(`/book-room?roomNumber=${roomNumber}`);
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {rooms.map((room) => (
          <div key={room._id} className="bg-white shadow-lg rounded-lg p-6">
            ...
            <p className="text-gray-700"><strong>Room Number:</strong> {room.roomNumber}</p>
            <p className="text-gray-700"><strong>Price:</strong> ₹{room.price}</p>
            <p className="text-gray-700"><strong>Occupancy:</strong> {room.occupancy}</p>
            <button
